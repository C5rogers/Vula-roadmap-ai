...
<template>
  ...
  >
    <div class="flex flex-col items-center gap-3">
      ...
      />
      <span
        class="text-stone-500 dark:text-stone-400 text-xs font-semibold uppercase tracking-wider"
        >Redirecting to learning center...</span
      >
    </div>
  </div>
  ...
  >
    ...
    <div
      ...
      class="flex h-[60vh] flex-col items-center justify-center text-center py-20 px-4 space-y-6 animate-fade-in"
    >
      ...
      <div class="space-y-2">
        ...
        <p
          class="text-muted text-sm max-w-md mx-auto h-8 flex items-center justify-center font-medium text-primary"
        >
          {{ generationMessage || "Connecting with Gemini AI..." }}
        </p>
      </div>
      <div
    ...
    <!-- --- Case 2: Loading Profile State --- -->
    <div
      v-else-if="profileQuery.status.value === 'pending'"
    ...
    <!-- --- Case 3: Profile Load Error State --- -->
    <UAlert
    ...
    <!-- --- Case 4: Step-by-Step Onboarding Wizard --- -->
    <div
      v-else-if="!profileQuery.data.value || isCreatingRoadmap"
      class="mx-auto max-w-2xl py-6"
    >
      <UCard class="shadow-md rounded-2xl border border-default">
        <template #header>
